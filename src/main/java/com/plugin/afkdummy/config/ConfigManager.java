...
public class ConfigManager {
    ...
    private long maxDurationMs = 86_400_000L;
    private java.util.List<Long> durationOptions = java.util.List.of(1800000L, 3600000L, 7200000L, 14400000L, 28800000L);
    ...
    public void reload() {
        ...
        if (jsonConfigFile != null && jsonConfigFile.exists() && jsonConfigFile.length() > 0) {
            try (Reader reader = new FileReader(jsonConfigFile, StandardCharsets.UTF_8)) {
                ...
                if (jsonRoot != null) {
                    ...
                    readDurations(getStringFromObj(settingsObj, "24h", "max-duration"), settingsObj.has("duration-options") ? gson.fromJson(settingsObj.get("duration-options"), String[].class) : new String[]{"30m", "1h", "2h", "4h", "8h"});
                    loadedFromJson = true;
        ...
        if (!loadedFromJson) {
            parseFromYaml();
            readDurations(plugin.getConfig().getString("settings.max-duration", "24h"), plugin.getConfig().getStringList("settings.duration-options").toArray(String[]::new));
    ...
    public java.util.List<Long> getDurationOptions() { return durationOptions; }

    private void readDurations(String maximum, String[] options) {
        try { maxDurationMs = com.plugin.afkdummy.util.DurationParser.parse(maximum, 365L * 86400000L); }
        catch (IllegalArgumentException e) { maxDurationMs = 86400000L; logger.warning("Invalid max-duration; using 24h."); }
        var values = new java.util.ArrayList<Long>();
        if (options.length == 0) options = new String[]{"30m", "1h", "2h", "4h", "8h"};
        for (String option : options) {
            try { long value = com.plugin.afkdummy.util.DurationParser.parse(option, maxDurationMs); if (!values.contains(value) && values.size() < 18) values.add(value); }
            catch (IllegalArgumentException e) { logger.warning("Ignoring duration option: " + option); }
        }
        if (values.isEmpty()) values.add(maxDurationMs);
        durationOptions = java.util.List.copyOf(values);
    }
    ...
    private void parseAndValidateSettings(JsonObject obj, String sourceFile) {
        ...
        int rawMaxServer = getIntFromObj(obj, 6, "max-server-wide-dummies", "max_server_wide_dummies", "max-server-dummies");
    ...
    private void parseFromYaml() {
        ...
        maxServerWideDummies = Math.max(1, cfg.getInt("settings.max-server-wide-dummies", 6));
    ...
    private void logConfigProperties() {
        ...
        logger.info("Free timed sessions: max-per-player=" + maxDummiesPerPlayer + ", max-server=" + maxServerWideDummies
                + ", max-duration=" + com.plugin.afkdummy.util.TimeUtil.formatDuration(maxDurationMs));
    }
