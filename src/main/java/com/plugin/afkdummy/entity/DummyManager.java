...
public class DummyManager {
    ...
    public DummySession spawnDummy(Player owner, Location location, long durationMs, String customName, String skinName) {
        if (!owner.hasPermission("afkdummy.use") || durationMs <= 0 || durationMs > config.getMaxDurationMs()) {
            owner.sendMessage("§cKeine Berechtigung oder ungültige Dauer (Maximum: " + com.plugin.afkdummy.util.TimeUtil.formatDuration(config.getMaxDurationMs()) + ").");
            return null;
        }
        ...
        try {
            ...
            data.setRemainingMillis(session.getRemainingTimeMs());
    ...
    public void handlePlayerJoin(Player player) {
        // Paper owns player-info and entity tracking; never inject duplicate spawn packets.
    }
