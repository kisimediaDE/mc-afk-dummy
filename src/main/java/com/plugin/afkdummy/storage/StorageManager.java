...
public class StorageManager {
        ...
        synchronized (fileLock) {
            try (Reader reader = new InputStreamReader(
                ...
                synchronized (dataList) {
                    dataList.clear();
                    if (loaded != null) {
                        for (DummyData data : loaded) {
                            if (data == null) throw new IllegalArgumentException("Null session");
                            data.getOwnerUUID();
                            if (data.getWorldName() == null || !Double.isFinite(data.getX()) || !Double.isFinite(data.getY()) || !Double.isFinite(data.getZ()) || !Float.isFinite(data.getYaw()) || !Float.isFinite(data.getPitch())) throw new IllegalArgumentException("Invalid location");
                            if (!data.hasRemainingMillis()) data.setRemainingMillis(data.getRemainingTimeMs());
                            dataList.add(data);
                        }
                    }
                }
                ...
                plugin.getLogger().log(Level.WARNING,
                        "Failed to load dummies.json; refusing to overwrite it.", e);
                synchronized (dataList) {
                    dataList.clear();
                }
                throw new IllegalStateException("Cannot load dummy storage", e);
            }
        }
    }
     ...
     */
    private void writeToFile(String jsonContent, long revision) {
        synchronized (fileLock) {
            ...
            try {
                ...
                plugin.getLogger().log(Level.SEVERE, "Failed to save dummies.json; previous file retained", e);
            }
        }
    }
