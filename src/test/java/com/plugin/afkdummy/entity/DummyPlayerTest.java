...
class DummyPlayerTest {
    ...
    class ProfileNameTests {
        ...
        void testFallback() throws Exception {
            ...
            assertTrue(profileName.startsWith("AFK_"));
        }

        @Test
        @DisplayName("Network identities never reuse visible owner names")
        void testExactNamePreservation() throws Exception {
            UUID session = UUID.randomUUID();
            assertNotEquals("John", invokeGenerateProfileName("John", session));
            assertNotEquals("JustRyt", invokeGenerateProfileName("JustRyt", session));
            assertNotEquals("Guard", invokeGenerateProfileName("Guard", session));
            assertNotEquals("Farmer", invokeGenerateProfileName("Farmer", session));
            assertNotEquals("Afkin", invokeGenerateProfileName("Afkin", session));
        }
