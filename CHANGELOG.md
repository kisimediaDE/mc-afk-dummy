...
and the locally developed `1.0.4-free-test.1` fork.

- Free timed sessions, configurable menu durations, `/spawn`, `/status`, `/remove`.
- Persisted remaining duration; server downtime pauses sessions; migration backups.
- Isolated skin properties, stable internal identities, Paper-managed entity tracking,
  consistent player registration and fake-connection cleanup.
- Removed the broken user-facing rename action, command and tab completion.
- Removed bStats from dependencies and replaced upstream-author support routing.
- Added Community metadata, README, provenance and license/dependency notices in the JAR.
- Retained technical name `AFKDummy`, data directory, commands and permissions for migration.
- Four-person live test of the prior testbuild reported successful by the server team;
  see `docs/ABNAHME.md` for the scope and separate Community build checks.
- Targets Paper 26.2 build 123 / Java 25. Paper 26.3 remains unsupported.

## Upstream history (retained verbatim)

The sections below describe upstream releases and their original claims, not
the current Community feature set or our independent verification.
